...
public struct Member: Identifiable, Codable, Hashable, Sendable {
    ...
    /// Discord's moderation timeout ("communication disabled until"). A
    /// future date means the member cannot send messages or speak in this
    /// guild until that time.
    ...
    public func isTimedOut(now: Date = .now) -> Bool {
        guard let communicationDisabledUntil else { return false }
        return communicationDisabledUntil > now
    }
