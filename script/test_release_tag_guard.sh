...
printf '%s\n' \
  ...
  '}' > "$TEMP_ROOT/Releases/v0.2.0-nightly.3.json"

"$ROOT_DIR/script/validate_release_tag.sh" \
  "$TEMP_ROOT" v0.2.0-nightly.3 >/dev/null
...
cp "$TEMP_ROOT/Releases/v0.2.0-nightly.3.json" "$HOOK_REPO/Releases/"
...
printf 'refs/heads/main %s refs/tags/v0.1.3 %s\n' "$HOOK_SHA" "$ZERO_SHA" \
  | (cd "$HOOK_REPO" && "$ROOT_DIR/script/pre_push_code_quality.sh") >/dev/null
printf 'refs/heads/main %s refs/tags/v0.2.0-nightly.3 %s\n' "$HOOK_SHA" "$ZERO_SHA" \
  | (cd "$HOOK_REPO" && "$ROOT_DIR/script/pre_push_code_quality.sh") >/dev/null
...
if printf 'refs/heads/main %s refs/tags/v0.2.0-nightly.4 %s\n' "$HOOK_SHA" "$ZERO_SHA" \
  | (cd "$HOOK_REPO" && "$ROOT_DIR/script/pre_push_code_quality.sh") >/dev/null 2>&1; then
