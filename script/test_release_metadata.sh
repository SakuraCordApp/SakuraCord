#!/usr/bin/env bash
set -euo pipefail

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd -P)"
# shellcheck source=release_metadata.sh
source "$ROOT_DIR/script/release_metadata.sh"

EXPECTED_NAME="MarrowChat.v0.1.0.dmg"
ACTUAL_NAME="$(marrowchat_release_dmg_name "0.1.0")"
ACTUAL_URL_NAME="$(marrowchat_release_dmg_url_name "0.1.0")"

if [[ "$ACTUAL_NAME" != "$EXPECTED_NAME" ]]; then
  echo "Unexpected release DMG name: $ACTUAL_NAME" >&2
  exit 1
fi
if [[ "$ACTUAL_URL_NAME" != "$EXPECTED_NAME" ]]; then
  echo "Unexpected release DMG URL name: $ACTUAL_URL_NAME" >&2
  exit 1
fi
if [[ "$ACTUAL_NAME" == *" "* || "$ACTUAL_NAME" == *"%"* ]]; then
  echo "Release asset names must not be normalized by GitHub." >&2
  exit 1
fi
if marrowchat_release_dmg_name "0.1" >/dev/null 2>&1; then
  echo "Invalid release versions must be rejected." >&2
  exit 1
fi

if ! marrowchat_is_release_tag "v0.1.0" \
  || ! marrowchat_is_release_tag "v0.2.0-Beta-7" \
  || marrowchat_is_release_tag "v0.2-Beta-7" \
  || marrowchat_is_release_tag "v0.2.0-beta.7" \
  || marrowchat_is_release_tag "v0.2.0-nightly.1"; then
  echo "Release tag classification is incorrect." >&2
  exit 1
fi
if [[ "$(marrowchat_release_track_from_tag "v0.2.0")" != "regular" ]] \
  || [[ "$(marrowchat_release_track_from_tag "v0.2.0-Beta-7")" != "nightly" ]]; then
  echo "Release track classification is incorrect." >&2
  exit 1
fi
if [[ "$(marrowchat_release_version_from_tag "v0.2.0-Beta-7")" != "0.2.0" ]]; then
  echo "Nightly tags must preserve the base bundle version." >&2
  exit 1
fi
if [[ "$(marrowchat_release_dmg_name_from_tag "v0.2.0-Beta-7")" \
  != "MarrowChat-v0.2.0-Beta-7.dmg" ]]; then
  echo "Nightly release DMG names must remain tag-specific." >&2
  exit 1
fi
if [[ "$(marrowchat_release_display_name_from_tag "v0.2.0-Beta-7")" \
  != "v0.2.0 Beta 7" ]]; then
  echo "Nightly release names must use human-readable beta naming." >&2
  exit 1
fi

printf 'Release metadata tests passed.\n'
