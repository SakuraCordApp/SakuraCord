...
if [[ -n "$CURRENT_HOOKS_PATH" && "$CURRENT_HOOKS_PATH" != "$EXPECTED_HOOKS_PATH" ]]; then
  ...
  echo "Integrate $ROOT_DIR/.githooks/pre-push into the existing hooks path manually." >&2
  exit 1
fi
...
echo "Every git push will validate committed tips and staged Swift changes first."
