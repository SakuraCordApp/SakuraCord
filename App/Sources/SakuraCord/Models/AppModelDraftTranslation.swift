...
extension AppModel {
    ...
    func translateDraft(in destination: MessageComposerDestination) {
        ...
        let language = translation.settings.resolvedDraftLanguage
    ...
    private func draftTranslationRefusal(for destination: MessageComposerDestination) -> TranslationError? {
        guard translation.settings.isEnabled else { return .notConfigured }
        let text = draftText(for: destination)
    ...
    private func startDraftTranslation(
        _ text: String,
        into language: TranslationLanguage,
        ...
        translation.drafts[destination] = DraftTranslation(
            ...
            requestRevision: draftRevision(for: destination)
        )
        ...
        state.draftTasks[destination] = Task { [weak self] in
            ...
            do {
                ...
                outcome = try .success(protector.restore(result.text, mode: .strict))
