...
struct NativeTimelineTranslationPresentation {
    ...
    static func make(
        ...
        switch entry.status {
        case .loading:
            return Self(requestID: entry.requestID, kind: .loading, caption: "Translating…", actionTitle: nil, text: nil)
        case let .failed(message):
            return Self(requestID: entry.requestID, kind: .failed, caption: "Couldn’t translate: \(message)", actionTitle: "Dismiss", text: nil)
...
extension NativeTimelineRowLayout {
    struct TranslationRegion {
        ...
        let regionID: NativeTimelineTextRegion
    ...
    static func translation(
        ...
        return TranslationRegion(
            messageID: messageID,
            regionID: .translation(presentation.requestID),
...
extension NativeTimelineRowPainter {
    static func drawMessageTranslation(_ input: NativeTimelineMessageDrawInput) {
        ...
        attributedText(
            ...
            model: input.model,
            selectionRange: input.textSelection?.itemIdentifier == .message(input.row.identity)
                && input.textSelection?.region == region.regionID ? input.textSelection?.range : nil,
            hoveredMentionCharacterIndex: input.hoveredMention?.itemIdentifier == .message(input.row.identity)
                && input.hoveredMention?.region == region.regionID ? input.hoveredMention?.characterIndex : nil,
            hoveredLinkCharacterIndex: input.hoveredTextLink?.itemIdentifier == .message(input.row.identity)
                && input.hoveredTextLink?.region == region.regionID ? input.hoveredTextLink?.characterIndex : nil,
            hoveredSpoilerRangeLocation: input.hoveredTextSpoiler?.itemIdentifier == .message(input.row.identity)
                && input.hoveredTextSpoiler?.region == region.regionID ? input.hoveredTextSpoiler?.rangeLocation : nil,
            revealedSpoilerLocations: input.revealedTextSpoilerState.locations(in: region.regionID)
        )
    }
}
