...
extension AppModel {
    ...
    func markAllRead() {
        guard !runsChatPerformanceBenchmark else { return }
        if let guilds = snapshot?.guilds {
            for guild in guilds where readState.guildUnread(guild.id) {
                markGuildRead(guild.id)
            }
        }
        let unreadDirectChannelIDs = readState.entries.values
            .filter { entry in
                (entry.kind == .directMessage || entry.kind == .groupDirectMessage)
                    && readState.unread(channelID: entry.channelID)
            }
            .map(\.channelID)
        for channelID in unreadDirectChannelIDs {
            markConversationRead(channelID: channelID)
        }
    }
