...
nonisolated struct ConversationPermissionBasis: Sendable {
    ...
    let currentUserCommunicationDisabledUntil: Date?
}
...
final class AppModel {
    ...
    var threadMessages: [Message] = [] {
        didSet {
            ...
            NotificationCenter.default.post(
                name: .marrowchatMessageRowsDidChange,
                object: self
            )
        }
    }
    ...
    @ObservationIgnored var forwardDestinationHistoryDefaultsKey =
        "dev.marrowchat.forward-destination-history.signed-out"
    ...
    @ObservationIgnored var discordFrecencyUsageDeltasDefaultsKey =
        "dev.marrowchat.forward-frecency-deltas.signed-out"
    ...
    /// MarrowChat has resolved locally. Private channels do not require a
    ...
    @ObservationIgnored var database: MarrowChatDatabase?
    ...
    init(
        ...
        accountDatabaseFactory: ((AccountID) -> MarrowChatDatabase?)? = nil,
        ...
        discordNetworkDisabled =
            discordNetworkDisabledOverride
                ?? (launchMode == .offlineTesting
                    || ProcessInfo.processInfo.environment["MARROWCHAT_DISABLE_DISCORD_NETWORK"] == "1")
        self.restoresStoredSession = restoresStoredSession
        usesInsecureDebugCredentials =
            usesInsecureDebugCredentialsOverride
                ?? (launchMode == .normal
                    && Bundle.main.object(
                        forInfoDictionaryKey: "MarrowChatInsecureDebugCredentialsEnabled"
                    ) as? Bool == true)
        ...
        self.accountDatabaseFactory = accountDatabaseFactory ?? { accountID in
            try? MarrowChatDatabase(accountID: accountID)
        }
        persistsEmojiPreferences = launchMode == .normal
        favoriteEmojiKeys =
            launchMode == .normal
                ? Set(UserDefaults.standard.stringArray(forKey: "dev.marrowchat.favorite-emojis") ?? [])
                : []
        emojiUsageCounts =
            launchMode == .normal
                ? UserDefaults.standard.dictionary(forKey: "dev.marrowchat.emoji-usage")
                as? [String: Int]
                ?? [:]
                : [:]
