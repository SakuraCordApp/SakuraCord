...
nonisolated enum ConversationAccess: Equatable, Sendable {
    ...
    var isReadable: Bool {
        switch self {
        case .readable, .timedOut: true
        case .checking, .hidden: false
        }
    }
}
...
nonisolated enum ConversationPermissionResolver {
    ...
    /// Discord's moderation timeout blocks sending independent of channel
    /// permissions. Downgrade an otherwise-sendable access to `.timedOut`
    /// when the member's timeout has not yet expired.
    static func applyingCommunicationTimeout(
        _ access: ConversationAccess,
        communicationDisabledUntil: Date?,
        now: Date = .now
    ) -> ConversationAccess {
        guard access.canSend,
              let communicationDisabledUntil,
              communicationDisabledUntil > now
        else { return access }
        return .timedOut(until: communicationDisabledUntil)
    }
