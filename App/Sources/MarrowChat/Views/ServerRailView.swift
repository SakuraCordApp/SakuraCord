...
struct ServerRailView: View {
    ...
    private var hasAnyUnread: Bool {
        home.isUnread || items.contains { item in
            switch item {
            case .guild(let entry):
                (entry.presentation?.guild.unreadCount ?? 0) > 0
            case .folder(let entry):
                entry.hasUnreadGuild
            }
        }
    }
...
private struct ReadAllRailButton: View {
    let isEnabled: Bool
    let action: () -> Void
    @State private var isHovering = false
    ...
    var body: some View {
        HStack(spacing: 5) {
            ...
            Button(action: action) {
                Image(systemName: "checkmark.circle.fill")
                    .font(.title2)
                    .frame(width: 44, height: 44)
                    .background(Color.secondary.opacity(0.16), in: ConcentricRectangle(cornerRadius: 14, style: .continuous))
            }
            .buttonStyle(.plain)
            .disabled(!isEnabled)
        ...
        .contentShape(Rectangle())
        .onHover { isHovering = $0 }
        .help("Mark all channels as read")
    }
}
