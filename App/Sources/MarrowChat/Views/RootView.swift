...
private struct ChatRootView: View {
    ...
    var body: some View {
        ...
        .alert("MarrowChat", isPresented: Binding(get: { model.errorMessage != nil }, set: {
            if !$0 {
                model.dismissError()
            }
        })) {
        ...
        .onReceive(NotificationCenter.default.publisher(for: .marrowchatToggleInspector)) { _ in model.showInspector.toggle() }
        .onReceive(NotificationCenter.default.publisher(for: .marrowchatNotificationDeepLink)) { notification in
