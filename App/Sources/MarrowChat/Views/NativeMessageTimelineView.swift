...
extension NativeMessageTimelineCoordinator {
            ...
            (NativeMessageTimelineView, NSScrollView) -> Void
        {
                    ...
                    if !applyJournalUpdate(
                            ...
                            != performanceFallbackReason
                        {
                            ...
                            Self.performanceLogger.notice(
                                """
                                MarrowChat timeline fallback: \(self.performanceFallbackReason, privacy: .public); \
            ...
            if didMutateItems {
                ...
                if parent.runsPerformanceAutoScroll {
                    ...
                    if updateMilliseconds >= 4 {
                        NSLog(
                            "MarrowChat timeline phases: %@ (%@) reconcile %.2f ms; metadata %.2f ms; origins %.2f ms; snapshot %.2f ms",
                            performanceUpdatePath,
                            performanceFallbackReason,
            ...
            if parent.runsPerformanceAutoScroll {
                ...
                if milliseconds >= 4 {
                    NSLog(
                        "MarrowChat timeline reload: %.2f ms (%d -> %d items)",
                        milliseconds,
                        oldItemCount,
                        items.count
                    )
