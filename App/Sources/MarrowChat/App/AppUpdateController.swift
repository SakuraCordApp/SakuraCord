...
nonisolated struct AppUpdateConfiguration: Equatable, Sendable {
    static let canonicalBundleIdentifier = "dev.marrowchat.MarrowChat"
    static let enabledInfoKey = "MarrowChatUpdatesEnabled"
    static let nightlyFeedInfoKey = "MarrowChatNightlyFeedURL"
    static let expectedFeedURL = URL(
        string: "https://github.com/d-lab17/MarrowChat/releases/latest/download/appcast.xml"
    )!
    static let expectedNightlyFeedURL = URL(
        string: "https://raw.githubusercontent.com/d-lab17/MarrowChat/nightly-feed/appcast.xml"
    )!
...
final class AppUpdateController: NSObject, ObservableObject, SPUUpdaterDelegate {
    ...
    var availabilityDescription: String {
        ...
        if canCheckForUpdates {
            return "MarrowChat is ready to check the signed \(releaseTrack.title.lowercased()) feed."
        }
