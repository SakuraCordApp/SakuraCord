...
`vMAJOR.MINOR.PATCH-nightly.NUMBER` publishes a GitHub prerelease to the nightly
track. Both build and sign the app, package a tag-specific DMG, generate and
verify the Sparkle appcast, publish reviewed release notes, and post the
...
{
  "schemaVersion": 1,
  "tagName": "v0.2.0-nightly.1",
  "githubDescription": "SakuraCord v0.2.0 nightly 1 adds ...",
...
updates the signed feed on the generated `nightly-feed` branch. Do not edit or
commit to that branch by hand. Switching tracks in the app checks the newly
selected feed through a silent Sparkle information check and remains silent
when no newer build exists.
