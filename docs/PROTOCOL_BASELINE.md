...
| `POST /attachments/refresh-urls` | Read-only retry after a Discord attachment image GET returns `403` or `404`; `attachment_urls` contains only the failed HTTPS Discord CDN URL. Accept a returned `refreshed_urls` entry only when its `original` matches the request and its refreshed URL has a trusted Discord media host and the same path. The signed URL is used only for the unauthenticated media download; media bytes are cached under the original URL. | Discord's public signed attachment CDN URL documentation and authenticated SakuraCord playback of a historical GIF on 24 September 2026. This route was not separately captured from the current official client. |
...
- unauthenticated HTTPS media GETs to `cdn.discordapp.com` and
  ...
  third-party link previews. A `403` or `404` for an attachment image can
  request a fresh signed URL through the authenticated route above; the CDN
  retry still carries no account credentials.
